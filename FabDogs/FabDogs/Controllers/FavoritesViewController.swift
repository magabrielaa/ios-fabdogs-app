...
class FavoritesViewController: UIViewController, UISearchResultsUpdating {
    ...
    func fetchDogs () {
            ...
            confirmedService.getDogs(completion: { dogs, error in
                ...
                // Filter dogs array by favorites from user defaults
                let favoriteDogs = dogs.filter { $0.isFavorite }
                
                // Sort the dogs array by name in alphabetical order
                self.myDogs = favoriteDogs.sorted(by: { $0.name < $1.name })
    ...
    func updateFavoriteSelection(cell: UITableViewCell) {
        ...
        tableView.reloadRows(at: [indexPathTappedOn], with: .none)
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        guard
            ...
            // Get cell located at the index path and downcast it to FavoriteDogCell
            let confirmedCell = self.tableView.cellForRow(at: selectedIndexPath) as? FavoriteDogCell
            else { return }
...
extension FavoritesViewController: UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if self.myDogs.isEmpty {
            let cell = UITableViewCell()
            cell.textLabel?.text = "No dogs have been saved as favorites"
            return cell
        } else {
            // (1) get cell
            let cell = self.tableView.dequeueReusableCell(withIdentifier:"dogCell") as! FavoriteDogCell // Force downcast to FavoriteDogCell
